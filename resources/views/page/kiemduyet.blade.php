            ...
            <table class="table" id="accountTable">
                ...
                <tbody id="bodytabletonvinh">
                    ...
                    <tr style="text-align: center"  id="data{{ $item[0]->id }}">
                        ...
                        <td>
                            {{-- //Truyen vao id --}}
                            <input class="btn btn-primary" type="submit" onclick="applytonvinh({{ $item[0]->id }})" value="Apply">  {{-- //Truyen vao id --}}
                        </td>
                    </tr>
                    @endforeach
                </tbody>
        ...
        <div class="container">
            <div class="row">
              <div class="col text-center">
                <button type="submit" onclick="apply_check()"  class=" btn btn-primary btn-lg">Apply_check</button>
              </div>
            </div>
          </div>
          </div>
...
<script>
    ...
    function applytonvinh(id) //id truyen
    {
        var mucsetup=$('#selecttonvinh'+id+' :selected').text(); //lay mucsetup la muc dang select voi id muc tra ve kieu du lieu text
            var fromdata= new FormData(); //de truyen dung Formdata
            fromdata.append('id',id);  //append : dua vao id
            fromdata.append('mucsetup',mucsetup);
                $.ajaxSetup({   //phan ajac
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')  //truyen du lieu tuong tu nhu crsf
                    }
                    });
                $.ajax({   //truyen
                    type: "POST", //phuong thuc truyen methot la post
                    url: " {{ route('kiemduyet.xulyimport_one') }} ",  //chay den duong dan xu ly o controller request hung
                    data: fromdata, //ten du lieu dua vao
                    contentType:false,
                    processData:false,
                    dataType: "JSON",  //bien tra ve mong muon
                    success: function (response)  //bien tra ve
                    {
